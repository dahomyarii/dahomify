...
def cmd_dev(args: argparse.Namespace) -> None:
    ...
    if action == "scaffold-lang":
        ...
        return
    if action == "cursor-task":
        _cmd_cursor_task(args)
        return
    print(
        colorize("Unknown dev action. Use `dahomify dev scaffold-lang` or `dahomify dev cursor-task`.", "red"),
        file=sys.stderr,
    )
    sys.exit(1)
...
def _cmd_cursor_task(args: argparse.Namespace) -> None:
    """Print a ready-to-paste Cursor agent prompt driven by Dahomify.

    Usage:
      dahomify dev cursor-task --goal "Short description"
    """
    goal = getattr(args, "goal", "") or ""
    goal = goal.strip() or "<describe your goal here>"

    root = str(PROJECT_ROOT)

    prompt = f"""
You are a Cursor agent working on a codebase that uses Dahomify for code health.
...
You have shell access in the project root:

- Dahomify (installed as `dahomify` / `python -m desloppify`):
  - `dahomify status`                # overall health + AGENT PLAN
  - `dahomify next --count 10`       # top 10 prioritized findings
  - `dahomify scan --path .`         # rescan project when status says tool changed
  - `dahomify show security --status open`
  - `dahomify show smells --status open`

- Project root (detected): {root}
- Project runbook / commands (if present): SHARIKLY_COMMANDS.md or similar.

If you also see npm or Python helpers in this repo (e.g. package.json, Makefile, scripts/run.js),
you may call them as well, but Dahomify should drive your priority order.
...
2. Read:
   - The score summary and "AGENT PLAN".
   - The biggest drags (e.g. Test health, Security, Elegance).
   - The structural-debt-by-area table.

3. Choose tasks that:
   - Are safe and concrete to implement.
   - Support the user goal above.
   - Respect Dahomify's priority ordering (security and correctness before cosmetics).

4. For each task:
   - Explain briefly why you chose it (tie back to Dahomify output and the user goal).
   - Make small, safe changes; add or update tests when reasonable.
   - Re-run relevant checks (tests, linters, Dahomify status/next/show).

5. After each completed task:
   - Re-run `dahomify next --count 10` and pick the next best item.
   - Stop or change direction when the user requests it or when you need more context.

## Important constraints

- Do not print or commit real secrets from any `.env` files.
- Prefer using Dahomify's plan (status/next/plan) as your source of truth for what to do next.
- Keep your changes small, reversible, and well-explained.
...
"""
    ...
    sys.stdout.write(prompt.lstrip())
