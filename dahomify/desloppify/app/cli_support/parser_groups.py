...
examples:
  dahomify scan
  dahomify scan --skip-slow
  dahomify scan --profile ci
  dahomify scan --force-resolve""",
...
examples:
  ...
  dahomify show unused --top 50
  dahomify show --chronic
  dahomify show --status all""",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p_show.add_argument(
        "pattern",
...
examples:
  dahomify next                       # single highest-priority item
  dahomify next --count 10            # top 10 items
  dahomify next --tier 1              # only tier 1
  dahomify next --group file          # group by file
  dahomify next --cluster my-cluster  # items in a cluster""",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
