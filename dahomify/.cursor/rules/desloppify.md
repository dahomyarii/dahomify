...
```bash
dahomify scan --path .       # analyse the codebase
dahomify status              # check scores — are we at target?
```
...
```bash
dahomify next                                        # shows the next workflow step
dahomify plan triage --stage observe --report "themes and root causes..."
dahomify plan triage --stage reflect --report "comparison against completed work..."
dahomify plan triage --stage organize --report "summary of priorities..."
dahomify plan triage --complete --strategy "execution plan..."
```

Then shape the queue. **The plan shapes everything `next` gives you** — don't skip this step.

```bash
dahomify plan                          # see the full ordered queue
dahomify plan reorder <pat> top        # reorder — what unblocks the most?
dahomify plan cluster create <name>    # group related issues to batch-fix
dahomify plan focus <cluster>          # scope next to one cluster
dahomify plan skip <pat>              # defer — hide from next
```

More plan commands:
```bash
dahomify plan reorder <cluster> top    # move all cluster members at once
dahomify plan reorder <a> <b> top     # mix clusters + findings in one reorder
dahomify plan reorder <pat> before -t X  # position relative to another item/cluster
dahomify plan cluster reorder a,b top # reorder multiple clusters as one block
dahomify plan resolve <pat>           # mark complete
dahomify plan reopen <pat>             # reopen
```
...
```bash
git checkout -b dahomify/code-health    # or dahomify/<focus-area>
```

**Set up commit tracking.** If you have a PR, link it for auto-updated descriptions:
```bash
dahomify config set commit_pr 42        # PR number for auto-updates
```

**The loop:**
```
1. dahomify next              ← what to fix next
2. Fix the issue in code
3. Resolve it (next shows you the exact command including required attestation)
4. When you have a logical batch, commit:
   git add <files> && git commit -m "dahomify: fix 3 deferred_import findings"
5. Record the commit:
   dahomify plan commit-log record      # moves findings uncommitted → committed, updates PR
6. Push periodically:
   git push -u origin dahomify/code-health
7. Repeat until the queue is empty
```

Score may temporarily drop after fixes — cascade effects are normal, keep going.
If `next` suggests an auto-fixer, run `dahomify autofix <fixer> --dry-run` to preview, then apply.
...
```bash
dahomify next --count 5                         # top 5 priorities
dahomify next --cluster <name>                  # drill into a cluster
dahomify show <pattern>                         # filter by file/detector/ID
dahomify show --status open                     # all open findings
dahomify plan skip --permanent "<id>" --note "reason" --attest "..." # accept debt
dahomify exclude <path>                         # exclude a directory from scanning
dahomify config show                            # show all config including excludes
dahomify scan --path . --reset-subjective       # reset subjective baseline to 0
```
...
Work on a dedicated branch named `dahomify/<description>` (e.g., `dahomify/code-health`, `dahomify/fix-smells`). Never push health work directly to main.
...
When Dahomify itself appears wrong or inconsistent:
...
`command -v dahomify >/dev/null 2>&1 && echo "dahomify: installed" || echo "NOT INSTALLED — run: pip install --upgrade git+https://github.com/peteromallet/desloppify.git"`
