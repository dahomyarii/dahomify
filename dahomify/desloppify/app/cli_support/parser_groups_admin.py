...
def _add_review_parser(sub) -> None:
    p_review = sub.add_parser(
        ...
        epilog="""\
examples:
  dahomify review --prepare
  dahomify review --run-batches --runner codex --parallel --scan-after-import
  dahomify review --external-start --external-runner claude
  dahomify review --external-submit --session-id <id> --import findings.json
  dahomify review --merge --similarity 0.8""",
...
Run with no subcommand to generate a full prioritized markdown plan.""",
        epilog="""\
typical workflow:
  dahomify scan                       # detect findings
  dahomify plan                       # full prioritized markdown
  dahomify plan queue                 # compact table of all items
  dahomify plan cluster create ...    # group related findings
  dahomify plan focus <cluster>       # narrow scope
  dahomify next                       # work on the next item
  dahomify plan done <id> --attest .. # mark as fixed
...
subcommands:
  ...
  cluster    Manage finding clusters""",
    ...
    p_move = plan_sub.add_parser(
        ...
        epilog="""\
examples:
  dahomify plan move security top                    # prioritize a detector
  dahomify plan move "unused::src/foo.ts::*" top     # prioritize by pattern
  dahomify plan move smells bottom                   # deprioritize smells
  dahomify plan move unused before security          # move before another""",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p_move.add_argument(
...
examples:
  ...
  dahomify plan done security --note "patched XSS" \\
...
def _add_dev_parser(sub) -> None:
    ...
    d_cursor = dev_sub.add_parser(
        "cursor-task",
        help="Print a ready-to-paste Cursor agent prompt that uses Dahomify status/next",
    )
    d_cursor.add_argument(
        "--goal",
        type=str,
        default="",
        help="Short description of what you want the agent to do",
    )
...
def _add_update_skill_parser(sub) -> None:
    p = sub.add_parser(
        "update-skill",
        help="Install or update the Dahomify skill/agent document",
    )
    p.add_argument(
        "interface",
        nargs="?",
...
def _add_agent_parser(sub) -> None:
    p = sub.add_parser(
        "agent",
        help="Run status, next, show security/smells, plan queue; write one file for Cursor/Claude. Use --synthesize for Claude plan.",
        epilog="""\
examples:
  dahomify agent
  dahomify agent --goal "add dark mode toggle"
  dahomify agent --goal "fix auth 401 handling" --scan
  dahomify agent --synthesize
  dahomify agent --out .cursor/context.md""",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    p.add_argument("--path", type=str, default=None, help="Project root (default: auto-detected)")
    p.add_argument("--goal", type=str, default=None, metavar="TEXT", help="What you want the agent to do; included in the context file as the user goal")
    p.add_argument("--no-expand-goal", action="store_true", help="Do not ask Claude to expand the goal into clearer steps (default: expand when ANTHROPIC_API_KEY is set)")
    p.add_argument("--scan", action="store_true", help="Run full scan before gathering context")
    p.add_argument("--synthesize", action="store_true", help="Call Claude to produce agent_plan.md (requires ANTHROPIC_API_KEY)")
    p.add_argument("--out", type=str, default=None, metavar="FILE", help="Output file (default: .desloppify/agent_context.md)")
