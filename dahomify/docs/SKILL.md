...
**The main thing you do is run `dahomify next`** — it tells you exactly what to fix and how. Fix it, resolve it, run `next` again. Keep going.
...
```
1. dahomify next              ← tells you exactly what to fix next
...
If `next` suggests an auto-fixer, run `dahomify fix <fixer> --dry-run` to preview, then apply.

**To be strategic**, use `plan` to shape what `next` gives you:
```bash
dahomify plan                        # see the full ordered queue
dahomify plan move <pat> top         # reorder — what unblocks the most?
dahomify plan cluster create <name>  # group related issues to batch-fix
dahomify plan focus <cluster>        # scope next to one cluster
dahomify plan defer <pat>            # push low-value items aside
dahomify plan skip <pat>             # hide from next
dahomify plan done <pat>             # mark complete
...
dahomify plan skip --permanent "<id>" --note "reason" # accept debt (lowers strict score)
